...
def conf_eval(model,loader,prop_threshold=0.50):
    ...
    for p in all_predictions:
        lstm_long = p['prob_up']  > prop_threshold
        lstm_short = p['prob_up']  < (1-prop_threshold)
        predict_long = (lstm_long)

                # Short: oba modele przewidują spadek
        predict_short = (lstm_short)
...
def grid_search(model_type,dataset_dir):
    ...
    # only run Boruta for MLP (flat features make sense)
...
def create_sample_weights(
    ...
    return weights
def weighted_accuracy(y_true, y_pred, sample_weights):
    """
    Accuracy weighted by move magnitude.
    Correct prediction on 3% move counts more than on 0.1% move.
    Consistent with WeightedBCELoss during training.
    """
    correct = (y_pred == y_true).astype(float)
    return np.average(correct, weights=sample_weights)
def train_with_params(dataset_dir,hidden_size,num_layers,dropout,model_type="CNN",SEED = 42, EPOCHS = 120, BATCH = 512, LR = 1e-4):
    ...
    train_metrics,result_train = conf_eval(model, train_loader, 0.6)
    test_metrics,result_test = conf_eval(model, val_loader, 0.6)
...
def Train_val(dataset_dir,SEED = 42, EPOCHS=120, BATCH=256, LR=1e-4):
    ...
    class_criterion = WeightedBCELoss(
        scale=60.0, min_w=0.10, max_w=3.0
    )
    # Increased gamma
    train_loader = DataLoader(train_ds, batch_size=BATCH,shuffle=True)
    val_loader = DataLoader(val_ds,batch_size=BATCH)
    ...
    print(
        f" MLP train: {train_brier:.3f}  test: {test_brier:.3f}  brier gap : {train_brier - test_brier:.3f}  weighted LSTM train: {weighted_train:.3f}  test: {weighted_test:.3f}  gap: {weighted_train - weighted_test:.3f}")
